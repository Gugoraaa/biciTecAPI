...
export const updateReportStatus = async (req: Request, res: Response) => {
    try {
        ...
        if (!status || !['Open', 'InProgress', 'Done'].includes(status)) {
            return res.status(400).json({
                success: false,
                message: "Status is required and must be 'Open', 'InProgress', or 'Done'"
            });
        }

        const updatedReport = await updateReportStatusModel(Number(id), status as 'Open' | 'InProgress' | 'Done');
...
export const updateReportPriority = async (req: Request, res: Response) => {
    try {
        ...
        const updatedReport = await updateReportPriorityModel(Number(id), priority as 'Low' | 'Medium' | 'High');
